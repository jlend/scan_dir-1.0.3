...
class plgSystemScanDir extends JPlugin
{
	PUBLIC FUNCTION onBeforeRender()
	{
		...
		// Выбор директории на запись файла с хеш-суммами (в корень или выше)	
		if ($this->params->get('scandir_save_dir', '1') == '1')
		{
			$save_dir = $scandir;
		}
		else
		...
		// Время интервал сканирования
		if(file_exists($save_dir . ".{$name}"))
		{
			$file_time = filemtime($save_dir . ".{$name}");			
...
//			echo "<html><pre>";
			...
			if(file_exists($save_dir . $datafilename))
			{
				$datafile = fopen($save_dir . $datafilename, "r");
...
//			echo $changes;
			...
			$datafile = fopen($save_dir . $datafilename, "w");
